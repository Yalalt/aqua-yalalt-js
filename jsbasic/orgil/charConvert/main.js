...
const toBigSmallCharacter = function(value, strLength) {
    ...
    for(let i = 0; i < strLength; i++){
        if(value.charCodeAt(i) > 64 && value.charCodeAt(i) < 91 && value.charCodeAt(i) !== 32) {
            result += String.fromCharCode(value.charCodeAt(i) + 32);
        } else if(value.charCodeAt(i) > 96 && value.charCodeAt(i) < 123 && value.charCodeAt(i) !== 32) {
            result += String.fromCharCode(value.charCodeAt(i) - 32);
        } else {
...
const upLowerCaseChanger = function(strInput){
    ...
    for(let j = 0; j < strInput.length; j++){
        // for(let shck = 0; shck < upperCaseFormat.length; shck++){
